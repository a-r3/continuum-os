...
| `governance/CONTEXT_RETRIEVAL_PROTOCOL.md` | Governance (proposed) | Proposed Context Retrieval Protocol defining retrieval-mode selection, source authority order, file-category rules, freshness checks, generated-context handling, and escalation rules; subordinate to ratified `CONTEXT_POLICY.md` and `CONTEXT_BUDGET.md` | Proposed Draft / Not Ratified | `CONT-P02-ONB-001` | NOT_RATIFIED | — | Product Owner | Proposed P02 context-retrieval deliverable draft; not canonical until Product Owner ratification. |
