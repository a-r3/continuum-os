...
- Recommendation R-2 `status: proposed` — **Security/Trust**: stage a path from Option S-A → Option S-B → (post-P04) Option S-C, with hook activation gated by fixture pass evidence (`54 pass / 0 fail`) and explicit Product Owner activation per `governance/CHANGE_PROTOCOL.md`. Rationale: matches ratified hook-activation discipline; preserves layered-not-absolute representation; aligns with Authority/Mutation/Evidence/Context/Secret/Agent requirement structure.
...
- status: `proposed`.
- context: `DEC-P02-SECURITY-TRUST-REQUIREMENTS-001` surfaces candidate Authority/Mutation/Evidence/Context/Secret/Agent requirements; hooks remain advisory.
- decision (proposed): stage Option S-A → Option S-B → Option S-C (post-P04) per Recommendation R-2, with hook activation gated by fixture pass evidence and explicit Product Owner activation.
- consequences (proposed): codifies advisory baseline; clears a path to activated hooks under existing discipline; honest layering ("no single layer is complete isolation") preserved.
- alternatives considered: stay on Option S-A indefinitely; jump straight to Option S-C without staged activation.
