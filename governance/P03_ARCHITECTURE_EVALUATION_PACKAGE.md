...
- Recommendation R-4 `status: proposed` (Step E-revised under `DEC-P03-ARCH-REVISION-002`; recommendation itself remains `status: proposed` until separately ratified under the `DEC-P03-ARCH-RECO-NNN` family; the corresponding ADR `ADR-DRAFT-P03-004` likewise remains `status: proposed` until separately ratified under the `DEC-P03-ARCH-ADR-NNN` family per `I-2`, and only after the underlying `R-4` recommendation is separately ratified) — **Context**: adopt Option X-B (lightweight runtime context pack) as the preferred future direction, conditional on a future separately approved generation gate (source-package Gate G-6) that demonstrably satisfies each of the following eight explicit runtime-context-pack acceptance criteria, per `governance/P03_ARCHITECTURE_RATIFICATION_PLAN.md` §6 E-7:
  1. **Generator provenance** — the pack must record which generator (tool, version, invocation) produced it and under which ratified gate, so provenance is traceable and non-anonymous.
  2. **Freshness check** — the pack must carry an explicit freshness check comparing pack-generation state against current canonical governance sources.
  3. **Staleness halt evidence** — when the freshness check detects staleness, reliance on the pack must demonstrably halt or flag with observable, recorded halt evidence, not silent continuation.
  4. **Subordination clause** — the pack must explicitly state it is subordinate to canonical governance sources; canonical-wins discipline (P-4 of `governance/CONTEXT_POLICY.md`) governs any conflict.
  5. **Hard token cap** — the pack must be bounded by an explicit, enforced maximum token budget consistent with `governance/CONTEXT_BUDGET.md`.
  6. **Observable generation** — pack generation must be an observable, auditable action, never silent or automatic, consistent with `governance/NEVER_AUTOMATIC.md` (P-9).
  7. **Reversibility** — pack generation must be reversible (deletable and regenerable) without loss of, or divergence from, canonical governance content.
  8. **Explicit non-elevation** — the pack must never be treated as canonical authority and must never be elevated above the `CLAUDE.md` cold-start reading order.

  This Step E revision maps the eight acceptance criteria to the following `P03-blocking` source-register rows per `governance/P03_G4_CLASSIFICATION_PLAN.md`: `OQ-P02-D-005` (`Generated Context Pack` naming/scope/boundary — §5 line 101), `OQ-P02-C-004` (Context-Bound integration into ratified context-policy set — §5 line 118), `P02-RISK-003` (Generated Context Treated as Canonical — §6 line 132), `P02-RISK-004` (Stale Generated Context — §6 line 133), and `P02-ROAD-006` (Decide Generated-Context Pack Naming, Scope, Freshness, Trust Boundary — §7 line 159). No source-register row is mutated by this revision; final per-row disposition remains a separately approved Product Owner gate. Until source-package Gate G-6 is separately approved and satisfied, Option X-A (cold-start full read) remains in effect. **This revision does not authorize runtime context pack generation under any condition.** Rationale: matches ratified `CONTEXT_POLICY.md` / `CONTEXT_BUDGET.md` / `CONTEXT_RETRIEVAL_PROTOCOL.md`; addresses token-cost without elevating derived content; does not modify or supersede already-ratified `DEC-P03-ARCH-RECO-001..005`, `DEC-P03-ARCH-ADR-001..005`, or `DEC-P03-ARCH-REVISION-001`.
...
- status: `proposed` (Step E-revised under `DEC-P03-ARCH-REVISION-002`; ADR itself remains `status: proposed` until separately ratified under the `DEC-P03-ARCH-ADR-NNN` family per `I-2`, and only after the underlying `R-4` recommendation is separately ratified under the `DEC-P03-ARCH-RECO-NNN` family).
- context: `DEC-P02-CONTEXT-POLICY-001` / `DEC-P02-CONTEXT-BUDGET-001` / `DEC-P02-CONTEXT-RETRIEVAL-001` ratify policy/budget/retrieval; no runtime context pack exists. `OQ-P02-D-005`, `OQ-P02-C-004`, `P02-RISK-003`, `P02-RISK-004`, and `P02-ROAD-006` are the `P03-blocking` source-register rows directly conditioned by this Step E revision per `governance/P03_G4_CLASSIFICATION_PLAN.md`.
- decision (proposed): adopt Option X-B (lightweight runtime context pack) as preferred future direction, conditional on a separately approved generation gate (source-package Gate G-6) that demonstrably satisfies each of the eight explicit runtime-context-pack acceptance criteria recorded in §6 (R-4): generator provenance, freshness check, staleness halt evidence, subordination clause, hard token cap, observable generation, reversibility, and explicit non-elevation. Until Gate G-6 is separately approved and satisfied, Option X-A (cold-start full read) remains in effect. **This revision does not authorize runtime context pack generation under any condition.**
- consequences (proposed): clears a path to lower-token cold starts once Gate G-6 is separately satisfied; preserves canonical-wins discipline (P-4) on any conflict; preserves the never-automatic surface (P-9) — pack generation must remain an observable, non-automatic, reversible action; generation strictly Product Owner-gated; does not modify or supersede already-ratified `DEC-P03-ARCH-RECO-001..005`, `DEC-P03-ARCH-ADR-001..005`, or `DEC-P03-ARCH-REVISION-001`.
- alternatives considered: Option X-A (cold-start full read indefinitely; rejected as the sole path because it does not address token cost, though it remains the interim posture until Gate G-6); Option X-C (structured index + on-demand retrieval; rejected as the preferred direction pending further evidence, though not foreclosed by this revision).
